...
// coronaInputs extracts the (subject, CoronaEvidence) pair for an envelope's
// corona lane: subject = D, evidence = routing + serialized signature. This
// mirrors exactly what the warp relay path (verifyPQLanes) hands to the
// subject-agnostic Corona verifier.
func coronaInputs(env *warp.Envelope) ([]byte, warp.CoronaEvidence) {
	id := env.Message.ID()
	return id[:], warp.CoronaEvidence{
		ChainID:    env.Message.SourceChainID,
		KeyEraID:   env.Message.SourceKeyEraID,
		Generation: env.Message.SourceGeneration,
		Sig:        env.CoronaSig,
	}
}

func TestSerializeDeserializeCoronaSigRoundTrip(t *testing.T) {
	sig, gk := runPulsarCeremony(t, 3, 2, "round-trip")
	wire, err := SerializeCoronaSig(sig)
	require.NoError(t, err)
	require.NotEmpty(t, wire)
	parsed, err := DeserializeCoronaSig(wire)
	require.NoError(t, err)
	require.True(t, corona.Verify(gk, "round-trip", parsed))
}

func TestRingtailVerifierAcceptsValidPulse(t *testing.T) {
	...
	v := NewRingtailVerifier(&stubResolver{gk: gk, suiteID: string(warp.DefaultCoronaSuiteID)})
	require.NoError(t, v.VerifyRingtailThreshold(coronaInputs(env)))
}

func TestRingtailVerifierRejectsTamperedEnvelopeFields(t *testing.T) {
	env := envFixture(t, 7, 11)
	signing := warp.CoronaSigningBytes(env.Message.ID())
	...
	// Tamper with KeyEraID after signing — D changes, so the corona sig over
	...
	v := NewRingtailVerifier(&stubResolver{gk: gk, suiteID: string(warp.DefaultCoronaSuiteID)})
	require.ErrorIs(t, v.VerifyRingtailThreshold(coronaInputs(env)), ErrCoronaVerifyFailed)
}

func TestRingtailVerifierRejectsOnAbsentPulse(t *testing.T) {
	env := envFixture(t, 1, 1)
	v := NewRingtailVerifier(&stubResolver{suiteID: string(warp.DefaultCoronaSuiteID)})
	require.ErrorIs(t, v.VerifyRingtailThreshold(coronaInputs(env)), ErrCoronaAbsent)
}

func TestRingtailVerifierRejectsResolverError(t *testing.T) {
	env := envFixture(t, 1, 1)
	env.CoronaSig = bytes.Repeat([]byte{0xFF}, 32)
	v := NewRingtailVerifier(&stubResolver{err: errors.New("registry down")})
	require.ErrorIs(t, v.VerifyRingtailThreshold(coronaInputs(env)), ErrCoronaGroupKeyResolverFailed)
}

func TestRingtailVerifierRejectsNilGroupKey(t *testing.T) {
	env := envFixture(t, 1, 1)
	env.CoronaSig = bytes.Repeat([]byte{0xFF}, 32)
	v := NewRingtailVerifier(&stubResolver{gk: nil, suiteID: string(warp.DefaultCoronaSuiteID)})
	require.ErrorIs(t, v.VerifyRingtailThreshold(coronaInputs(env)), ErrCoronaGroupKeyResolverFailed)
}

func TestRingtailVerifierRejectsSuiteMismatch(t *testing.T) {
	env := envFixture(t, 1, 1)
	env.CoronaSig = bytes.Repeat([]byte{0xFF}, 32)
	_, gk := runPulsarCeremony(t, 3, 2, "x")
	v := NewRingtailVerifier(&stubResolver{gk: gk, suiteID: "Pulsar-SHA3-experimental"})
	require.ErrorIs(t, v.VerifyRingtailThreshold(coronaInputs(env)), ErrCoronaSuiteMismatch)
}

// TestCoronaSigningBytesBindsAllTranscriptFields proves the Corona subject
// (CoronaSigningBytes(D)) changes when ANY transcript field changes —
// because D folds them all.
func TestCoronaSigningBytesBindsAllTranscriptFields(t *testing.T) {
	env := envFixture(t, 7, 11)
	env.Message.SourceNebulaRoot = [32]byte{0x01, 0x02}
	base := warp.CoronaSigningBytes(env.Message.ID())

	mutate := func(f func(c *warp.Message)) []byte {
		c := env.Message
		f(&c)
		return warp.CoronaSigningBytes(c.ID())
	}
...
// TestCoronaSigningBytesPrefix proves the Corona subject carries the
// CORONA domain-separation tag.
func TestCoronaSigningBytesPrefix(t *testing.T) {
	env := envFixture(t, 1, 1)
	out := warp.CoronaSigningBytes(env.Message.ID())
	require.True(t, bytes.HasPrefix(out, []byte("LUX-WARP-ZAP-CORONA-v1")))
	d := env.Message.ID()
...
func TestHorizonFromEnvelopePopulatesAllLanes(t *testing.T) {
	...
	env.CoronaSig = bytes.Repeat([]byte{0xCC}, 16)
	...
	require.Equal(t, env.CoronaSig, h.CoronaRingtail)
