...
// and the canonical Message preimage matches `coreC14n`. Drift between
...
type Fixture struct {
	...
	MessageC14n string `json:"coreC14n"`    // hex of zap_c14n(Message) — the digest preimage
...
func emit(name string, networkID uint32, sourceChainID, token, recipient string, destChainID uint64, amount *big.Int, vaultIsZero bool, nonce uint64) (Fixture, error) {
	...
	message, err := warp.NewMessage(networkID, src, payload)
	if err != nil {
		return Fixture{}, err
	}
	...
	return Fixture{
		...
		MessageC14n:   "0x" + hex.EncodeToString(message.Bytes()),
